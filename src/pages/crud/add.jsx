...
import { Plus as BoxPlus, XCircle, CloudUpload, CardImage, InfoCircle } from "react-bootstrap-icons"
...
export default function AddProduct() {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      const res = await fetch("https://api.cloudinary.com/v1_1/duftqflob/image/upload", {
        method: "POST",
        body: formData,
      });
  ...
  const formFields = [
    ...
    { name: "category", label: "Category", placeholder: "e.g. Moisturizer, Cleanser" },
    { name: "categoryId", label: "Category ID", placeholder: "Category reference ID" },
    { name: "price", label: "Price ($)", placeholder: "0.00", type: "number" },
    { name: "stock", label: "Stock Quantity", placeholder: "0", type: "number" },
    { name: "discount", label: "Discount (%)", placeholder: "0", type: "number" },
    { name: "productType", label: "Product Type", placeholder: "e.g. Cream, Serum, Mask" },
    { name: "unit", label: "Unit", placeholder: "e.g. oz, ml, g" },
    { name: "sizes", label: "Available Sizes", placeholder: "e.g. 30ml, 50ml (comma separated)" },
    { name: "tags", label: "Tags", placeholder: "e.g. hydrating, anti-aging (comma separated)" }
  ];
  ...
  return (
    <Layout>
      <Card className="border-0 shadow-sm">
        <Card.Header className="bg-white border-bottom" style={{ borderLeft: `4px solid #cb1f28` }}>
          <h2 className="mb-0 py-2" style={{ color: '#cb1f28' }}>Add New Skincare Product</h2>
        </Card.Header>
        <Card.Body>
              ...
              <Col md={8}>
                  ...
                  <Card.Body>
                    ...
                    <Row>
                      {formFields.slice(0, 3).map((field) => (
                        <Col md={field.name === "name" ? 12 : 6} key={field.name}>
                ...
                <Card className="mb-4 border-0 shadow-sm">
                  <Card.Body>
                    ...
                    <Form.Label>Ingredients, Benefits, How to Use, etc.</Form.Label>
                  ...
                  <Card.Body>
                    ...
                    {product.images.length === 0 && (
                      <div className="text-center p-4 bg-light rounded mb-3">
                        ...
                        <p className="text-muted small mb-0">No images uploaded yet</p>
                      </div>
                    )}
                      ...
                      <div className="d-grid">
                        ...
                        >
                          <input
                            ...
                            accept="image/*"
                          />
                          <CloudUpload size={24} className="position-absolute" style={{ left: "1rem", top: "50%", transform: "translateY(-50%)" }} />
